...
class SMALLayer(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.register_buffer("shapedirs", torch.from_numpy(np.array(kwargs['shapedirs'], dtype=np.float32))) # [3889, 3, num_betas]
    ...
    def forward(
        ...
        if betas is None:
            betas = torch.zeros(
                [batch_size, self.shapedirs.shape[-1]], dtype=dtype, device=device)
        if transl is None:
