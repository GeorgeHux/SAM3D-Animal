...
def cam_full_to_crop(cam_full, box_center, box_size, img_size, focal_length=5000.):
    """
    Inverse of cam_crop_to_full.
    Convert full-image camera translation (tx, ty, tz) back to crop-space parameters (s, tx, ty).
    Args:
        cam_full: tensor [N, 3] in full image coords (tx_full, ty_full, tz_full)
        box_center: [N, 2] bbox center in full image coords
        box_size: [N] bbox size in full image
        img_size: [N, 2] (w, h)
        focal_length: scalar focal length used in projection
    Returns:
        cam_crop: tensor [N, 3] where first dim is scale s, followed by tx_crop, ty_crop
    """
    img_w, img_h = img_size[:, 0], img_size[:, 1]
    cx, cy, b = box_center[:, 0], box_center[:, 1], box_size
    w_2, h_2 = img_w / 2., img_h / 2.
    ...
    # Recover scale s from depth and bbox (matches cam_crop_to_full derivation)
    s = 2 * focal_length / (tz_full * b)
    bs = b * s + 1e-9  # equals 2 * f / tz_full

    tx_crop = tx_full - 2 * (cx - w_2) / bs
    ty_crop = ty_full - 2 * (cy - h_2) / bs

    cam_crop = torch.stack([s, tx_crop, ty_crop], dim=-1)
    return cam_crop


def cam_crop_s_to_t(cam_crop, img_res, focal_length=5000.):
    """
    Convert crop-space weak-perspective params (s, tx, ty) to crop camera translation (tx, ty, tz).
    Assumes crop image center at (img_res/2, img_res/2) and fx=fy=focal_length.
    """
    s, tx, ty = cam_crop[:, 0], cam_crop[:, 1], cam_crop[:, 2]
    # In crop coordinates, bbox size ~= img_res, so depth from weak-persp: tz = 2*f / (img_res * s)
