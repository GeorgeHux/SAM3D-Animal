...
class MeshRenderer:
    ...
    def visualize_wandb_for_multiple_animals(self, vertices, camera_translation, images, focal_length, pred_keypoints, gt_keypoints, images_gt_boxes, images_pred_boxes, pred_masks=None, gt_masks=None):
        images_np = np.transpose(images, (0, 2, 3, 1))
        rend_imgs = []
        pred_keypoints = [np.concatenate((self.img_res * (pred_keypoints[i] + 0.5), gt_keypoints[i][:, :, [-1]]), axis=-1)
                          for i in range(len(pred_keypoints))]
        for i in range(len(gt_keypoints)):
            gt_keypoints[i][:, :, :-1] = self.img_res * (gt_keypoints[i][:, :, :-1] + 0.5)
        for i in range(len(vertices)):
            rend_img_np = images_np[i].copy()
            rend_img_side_np = images_np[i].copy()
            pred_keypoints_img = images_np[i].copy()
            gt_keypoints_img = images_np[i].copy()
            for n in range(vertices[i].shape[0]):
                rend_img_np = self.__call__(
                    vertices[i][n],
                    camera_translation[i][n],
                    rend_img_np,
                    focal_length=focal_length,
                    side_view=False,
                )
                rend_img_side_np = self.__call__(
                    vertices[i][n],
                    camera_translation[i][n],
                    rend_img_side_np,
                    focal_length=focal_length,
                ...
                keypoints = pred_keypoints[i][n]
                pred_keypoints_img = render_keypoint(255 * pred_keypoints_img, keypoints) / 255
                keypoints = gt_keypoints[i][n]
                gt_keypoints_img = render_keypoint(255 * gt_keypoints_img, keypoints) / 255
            rend_img = torch.from_numpy(np.transpose(rend_img_np, (2, 0, 1))).float()
            rend_img_side = torch.from_numpy(np.transpose(rend_img_side_np, (2, 0, 1))).float()
            rend_imgs.append(torch.from_numpy(images[i]))
            rend_imgs.append(rend_img)
            rend_imgs.append(rend_img_side)
            rend_imgs.append(torch.from_numpy(pred_keypoints_img).permute(2, 0, 1))
            rend_imgs.append(torch.from_numpy(gt_keypoints_img).permute(2, 0, 1))
            rend_imgs.append(torch.from_numpy(images_gt_boxes[i]))
            rend_imgs.append(torch.from_numpy(images_pred_boxes[i]))
            if pred_masks is not None:
                # pred_masks[i]: [H, W] float32 in [0,1] → replicate to 3-channel for visualization
                pm = pred_masks[i]
                pm = torch.from_numpy(pm)
                pm = pm.float().unsqueeze(0).expand(3, -1, -1)  # [3, H, W]
                rend_imgs.append(pm)
            if gt_masks is not None:
                gm = gt_masks[i]
                gm = torch.from_numpy(gm)
                gm = gm.float().unsqueeze(0).expand(3, -1, -1)  # [3, H, W]
                rend_imgs.append(gm)
        return rend_imgs


class SilhouetteRenderer:
    # silhouette_renderer = base_renderer(size=256, focal=focal, device=device)
    def __init__(self, size, focal=None, fov=None, device='cpu'):
        ...
        self.R = torch.tensor([[-1, 0, 0],
                               [0, -1, 0],
                               [0, 0, 1]], dtype=torch.float32).repeat(1,1,1).to(device)
        self.T = torch.zeros(3, dtype=torch.float32).repeat(1, 1).to(device)

    def init_camera(self, focal, fov):
        if fov is None:
            fov = 2 * np.arctan (self.size/(focal * 2)) * 180 / np.pi
        camera = FoVPerspectiveCameras(zfar=350, fov=fov, device=self.device)
        return camera
    ...
    def init_silhouette_renderer(self):
        ...
        silhouette_renderer = Pytorch3dMeshRender(
    ...
    def __call__(self, vertices, faces):
        ''' Right now only render silhouettes and depths
            Input:
            vertices: BN * V * 3
            faces: BN * F * 3
        '''
        ...
        R = self.R.to(device)
        T = self.T.to(device)
        self.silhouette_renderer.rasterizer.cameras = self.camera.to(device)
        torch_mesh = Meshes(verts=vertices, faces=faces)
        silhouettes = self.silhouette_renderer(meshes_world=torch_mesh.clone(),
                                               R=R, T=T)[..., 3]
        return silhouettes