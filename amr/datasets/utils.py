...
def generate_image_patch_cv2(img: np.array, c_x: float, c_y: float,
    ...
    trans_worot = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, patch_width, patch_height, scale, 0)
    trans = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, patch_width, patch_height, scale, rot) if rot else trans_worot
    ...
    return img_patch, trans_worot, trans, img_border_mask
...
def get_example(img_path: Union[str, np.ndarray], center_x: float, center_y: float,
    ...
    is_multi = keypoints_2d.ndim == 3
    if is_multi:
        center_x_arr = np.asarray(center_x, dtype=np.float32)
        center_y_arr = np.asarray(center_y, dtype=np.float32)
        width_arr = np.asarray(width, dtype=np.float32)
        height_arr = np.asarray(height, dtype=np.float32)
        x1 = center_x_arr - width_arr / 2.0
        x2 = center_x_arr + width_arr / 2.0
        y1 = center_y_arr - height_arr / 2.0
        y2 = center_y_arr + height_arr / 2.0
        center_x = (x1.min() + x2.max()) / 2.0
        center_y = (y1.min() + y2.max()) / 2.0
        width = x2.max() - x1.min()
        height = y2.max() - y1.min()
    ...
    if do_extreme_crop and not is_multi:
    ...
    if is_multi:
        keypoints_3d = np.stack(
            [keypoint_3d_processing(kpts, rot, do_flip) for kpts in keypoints_3d],
            axis=0,
        )
    else:
    ...
    img_patch_cv, trans_worot, trans, img_border_mask = generate_image_patch_cv2(cvimg,
                                                   center_x, center_y,
                                                   width, height,
    ...
    if is_multi:
        num_animals = keypoints_2d.shape[0]
        processed_params = {k: [] for k in smal_params}
        processed_has = {k: [] for k in has_smal_params}
        for i in range(num_animals):
            per_params = {k: v[i] for k, v in smal_params.items()}
            per_has = {k: v[i] for k, v in has_smal_params.items()}
            per_params, per_has = smal_param_processing(per_params, per_has, rot, do_flip)
            for k in processed_params:
                processed_params[k].append(per_params[k])
            for k in processed_has:
                processed_has[k].append(per_has[k])
        smal_params = {k: np.stack(v, axis=0) for k, v in processed_params.items()}
        has_smal_params = {k: np.stack(v, axis=0) for k, v in processed_has.items()}
    else:
    ...
    if do_flip:
        if is_multi:
            for i in range(keypoints_2d.shape[0]):
                keypoints_2d[i] = fliplr_keypoints(
                    keypoints_2d[i], img_width, list(range(len(keypoints_2d[i])))
                )
        else:
    ...
    if is_multi:
        for i in range(keypoints_2d.shape[0]):
            for n_jt in range(len(keypoints_2d[i])):
                keypoints_2d[i, n_jt, 0:2] = trans_point2d(
                    keypoints_2d[i, n_jt, 0:2], trans
                )
    ...
    if not return_trans:
        return img_patch, keypoints_2d, keypoints_3d, smal_params, has_smal_params, img_size, img_border_mask, box_center, scale
    else:
        return img_patch, keypoints_2d, keypoints_3d, smal_params, has_smal_params, img_size, trans_worot, trans, img_border_mask, box_center, scale
