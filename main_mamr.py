...
import hydra
import pyrootutils
from amr.models.amr_multi import MultiAMR
from amr.datasets import MAMRDataModule
...
def main(cfg: DictConfig) -> Optional[float]:
    datamodule = MAMRDataModule(cfg)
    model = MultiAMR(cfg)
    wandb_dir = os.path.abspath(cfg.paths.output_dir)
    os.makedirs(wandb_dir, exist_ok=True)
    os.environ["WANDB_DIR"] = wandb_dir

    wandb_mode = cfg.WANDB.get("MODE", os.environ.get("WANDB_MODE", "offline"))
    ...
    wandb_logger_kwargs = dict(
        ...
        save_dir=wandb_dir,
        project=cfg.WANDB.get("PROJECT_NAME", "MultiAnimalRecon"),
    ...
    resume_id = cfg.WANDB.get("ID", None) or cfg.WANDB.get("RUN_ID", None)
    ...
    trainer: Trainer = hydra.utils.instantiate(
        ...
        accumulate_grad_batches=int(cfg.GENERAL.get("GRAD_ACCUM_STEPS", 1)),
    )
